...
if [ "$OS_NAME" = "Darwin" ]; then
    ...
    export $(grep -v '^#' windows.env | gxargs -d '\n')
else
    echo "Running on linux, using xargs"
    export $(grep -v '^#' windows.env | xargs -d '\n')
fi
...
# start 
docker build -f ./grafana/Dockerfile .. -t tardis-grafana
docker run -d --name tardis-grafana -p 3000:3000 --network=tools_network tardis-grafana:latest
# docker run -d --name=grafana \
#     -p 3000:3000 \
#     --network tools_network \
#     -v "$(pwd)"/grafana/dashboard.yaml:/etc/grafana/provisioning/dashboards/main.yaml \
#     -v "$(pwd)"/grafana/dashboards:/var/lib/grafana/dashboards \
#     -v "$(pwd)"/grafana/datasources.yaml:/etc/grafana/provisioning/datasources/main.yaml \
#     grafana/grafana
